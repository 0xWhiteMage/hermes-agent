...
- **Retrieval:** BM25 over tokenized tool name + description + parameter
  names, with Snowball stemming (English) applied to both the index and
  the query so morphological variants match ("issues" finds
  `create_issue`). Falls back to a literal substring match on the tool
  name when BM25 returns no positive-score hits.
