...
class TestShapePredicate:
    """cmd_gui and step_expose_cli must agree about what a bundle is."""
...
class TestCmdGuiOnABundle:
    ...
    def _run(monkeypatch, repo: Path, args, *, launcher_ok: bool = True):
        ...
        def record_npm_install(root, **kw):
        ...
        with patch("installation.nodejs.npm_install", side_effect=record_npm_install), \
             patch("hermes_cli.bundled_app.subprocess.Popen", side_effect=record_popen), \
    ...
    def test_launches_the_bundle_and_builds_nothing(self, tmp_path, monkeypatch):
        ...
        assert builds == []
        assert launches == [[str(tmp_path / "linux-unpacked" / "Hermes")]]
